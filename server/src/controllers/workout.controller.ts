...
export async function getWorkoutHistoryController(
  req: Request,
  res: Response<JsonApiResponse>,
  next: NextFunction
) {
  try {
    ...
    const workoutHistory = await prismaDB.workoutInstance.findMany({
      where: { userId: user.id },
      include: {
        workout: true,
        workoutExerciseInstances: {
...
export async function getAllWorkoutSetsController(
  req: Request,
  res: Response<JsonApiResponse>,
  next: NextFunction
) {
  try {
    ...
    const workoutSets = await prismaDB.workoutSetInstance.findMany({
      where: { userId: user.id },
      orderBy: { createdAt: 'desc' },
    });

    return res.status(200).json({ success: true, data: workoutSets });
  } catch (err) {
    next(err);
  }
}
